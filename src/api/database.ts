...
export namespace DatabaseOperations {
    export class SearchContent extends DatabaseOperation {
        constructor(searchParams: SearchParams) {
            const [term, order, user, region_code, itemId, category, count] = searchParams.params;
            super(DatabaseOperationName.searchContent, term, order, user, region_code, itemId, category, count);
        }
    }
    ...
    export class SearchAccountHistory extends DatabaseOperation {
        constructor(accountId: string, order: string, startObjecId: string = '0.0.0', limit = 100) {
            super(DatabaseOperationName.searchAccountHistory, accountId, order, startObjecId, limit);
        }
    }
    ...
    export class RestoreEncryptionKey extends DatabaseOperation {
        constructor(contentId: string, elGamalPrivate: string) {
            super(DatabaseOperationName.restoreEncryptionKey, {s: elGamalPrivate}, contentId);
        }
    }
...
export class DatabaseApi extends Database {
    ...
    public static create(config: DatabaseConfig,
                         api: any): DatabaseApi {
    ...
    public exec(operation: DatabaseOperation): Promise<any> {
        ...
        const a = new DatabaseOperations.SearchContent(new SearchParams('', '', '', '', '', '')).name;
        console.log(a);
